...
def create_pdf_contract(btc, pledge, thr_addr, fname):
    out = os.path.join(CONTRACTS_DIR, fname)
    ...
    c.drawString(1*inch, h-1.5*inch, f"BTC Address: {btc}")
    c.drawString(1*inch, h-1.8*inch, "Pledge Text:")

    text = c.beginText(1*inch, h-2.1*inch)
    text.setFont("Helvetica",12)
    line = ""
    for wrd in pledge.split():
        if len(line)+len(wrd)+1 <= 80:
            line += (" "+wrd if line else wrd)
        else:
            text.textLine(line)
            line = wrd
    if line:
    ...
    c.drawString(1*inch, h-2.1*inch - (len(pledge)//80 + 1)*15 - 10,
                 f"Generated THR Address: {thr_addr}")
    c.save()
    return out
...
def pledge_submit():
    ...
    if any(p["btc_address"]==btc_address for p in pledges):
        p = next(p for p in pledges if p["btc_address"]==btc_address)
        return jsonify(
            status="already_verified",
            thr_address=p["thr_address"],
            pledge_hash=p["pledge_hash"],
            pdf_filename=f"pledge_{p['thr_address']}.pdf"
        ),200

    txns = get_btc_txns(btc_address, BTC_RECEIVER)
    paid = any(tx.get("to")==BTC_RECEIVER and float(tx.get("amount_btc",0))>=0.00001
               for tx in txns)
    if not paid:
    ...
    pdfn = f"pledge_{thr_addr}.pdf"
    create_pdf_contract(btc_address, pledge_text, thr_addr, pdfn)
    return jsonify(
        status="verified",
        thr_address=thr_addr,
        pledge_hash=phash,
...
def submit_block():
    ...
    data["reward"]=r; data["pool_fee"]=fee; data["reward_to_miner"]=round(r-fee,6)
    chain.append(data)
